...
// Type: BundleTransformer.Core.Validators.StyleAssetTypesValidator
...
namespace BundleTransformer.Core.Validators
{
  ...
  public sealed class StyleAssetTypesValidator : IValidator
  {
    ...
    public void Validate(IList<IAsset> assets)
    {
      ...
      if (assets.Count == 0)
        return;
      IList<IAsset> list = (IList<IAsset>) assets.Where<IAsset>((Func<IAsset, bool>) (a => !a.IsStylesheet)).ToList<IAsset>();
      if (list.Count > 0)
      {
        string[] array = list.Select<IAsset, string>((Func<IAsset, string>) (a => a.VirtualPath)).ToArray<string>();
        throw new InvalidAssetTypesException(string.Format(BundleTransformer.Core.Resources.Strings.Assets_StyleAssetsContainAssetsWithInvalidTypes, (object) string.Join(", ", array)), array);
      }
    }
  }
}
